...
public class WikiWidgetService extends RemoteViewsService {
    ...
    public RemoteViewsFactory onGetViewFactory(Intent intent) {
        ...
        return new GeoWidgetViewFactory(this.getApplicationContext(), intent);
    }
...
class WikiRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    ArrayList<GeoName> returnGeos(InputStream iStream) {
    	String jsonStr = NetworkHelper.convertStreamToString(iStream);
    	ArrayList<GeoName> glist = new ArrayList<GeoName>();
    ...
    void networkTaskCode(double latitude, double longitude) {
    	...
    	//given a URL, fetch the web page and return a buffered input stream
     	...
     	try {
			...
			if ((rcode >= 200) && (rcode < 300)) {
     	    ...
     	    //returns the buffered stream or null
 	            ...
 	            Log.w(TAG, "Error while retrieving data from " + requestUrl + " " + e.toString());
 	        } finally {
    ...
    void createList( ) {
    	...
    	if(WikiWidgetsApp.geonames == null) {
    		final double[] gps = getGPS(mListContext);
...
*/    	
    ...
    public RemoteViews getViewAt(int position) {
        ...
        final RemoteViews rv = new RemoteViews(mListContext.getPackageName(), itemId);
